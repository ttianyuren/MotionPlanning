...
def play_commands(commands, use_control=0):
    if use_control:
...
def get_pddlstream_problem(scn):
    ...
    goal = ('and',
            ('AtConf', conf),
            ('On', scn.bd_body['cup'], scn.bd_body['region_shelf']),
...
def main(display=True, use_bo=0, visualization=1, new_problem=1):
    visual_UCT = 0

    connect(use_gui=visualization)

    PlanningScenario = get_scn(0)
    scn = PlanningScenario(enable_drops=False)
    ...
    if new_problem:
        ...
        op_plan = sk_batch.generate_operatorPlan(0)  # 3
    else:
